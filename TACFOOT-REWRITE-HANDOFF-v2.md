TACFOOT REWRITE HANDOFF — February 20, 2026 (v2)

This document captures decisions and complete game spec from the
diagnosis conversation and subsequent design discussions. No code.
No old variable names. This is the starting point for the design
doc thread.
...
Possession-based, not play-count.
  ...
  - 4 quarters
  - 2 player possessions per quarter = 8 possessions per game
  - Each drive lasts as many plays as it takes (score, punt,
    turnover, or turnover on downs)
  - After every possession change, the CPU gets the ball and
    the player picks a Defensive Posture (see below)
  - The CPU drive resolves, then the player's next possession
    starts

Quarter transitions:
  - Quarter advances when the player's possession count for
    that quarter is used up
...
6. DECISION (pocket phase — passing plays)
   ...
   pocket degrades. NOT two separate states.
   ...
   - Action buttons below throws:
     ...
     Throw It Away (carries grounding risk — see below)
     Tuck the Ball (accept sack, protect ball, zero fumble,
       minimal loss — the "it's over, don't make it worse" option)
   - Practice/Preseason: highlights recommended actions,
     shows explanatory tips about why targets are open
   - Scramble lane button (when pocket integrity is below 40%
     but at least one side has open field — the collapse is
     lopsided and the QB can escape the other way)
   ...
   30-60% pocket (yellow):
     ...
     Completion percentages start degrading from pressure.
   ...
   15-30% pocket (red):
     ...
     as recommended. Red bars. You still have a beat but it's
     ugly.
...
7. RPO READ (run-pass option on run plays)
   ...
   RPO scope — which run plays get the read:
     Inside Run, Outside Run, Counter, Power Run, Stretch Run
     NOT HB Toss (pitch is already in the air)
     NOT QB Sneak (pure push play, no read)
   ...
   RPO Quick Pass target:
     ...
     read — TE leaks out if the player picks Quick Pass. This
     mirrors how real RPOs work.
...
8. RUNNER MODE (ball carrier has the ball)
   ...
   Runner move labels vary by position:
     RB: Sprint, Hit the Hole, Juke, Dive Forward
     WR/TE after catch: Sprint, Cut Outside, Juke, Dive Forward
     QB scramble: Sprint Upfield, Slide, Cut Outside, Truck It

   Runner action success rates are contextual based on
   defender positions (see Contact System below).
...
13. DEFENSIVE POSTURE (single-player only)
    ...
    - Possible results:
        ...
        CPU turnover (no score, player gets ball at the
          turnover spot instead of own 25 — better position)
    ...
    Posture modifiers adjust these odds:
      Bend Don't Break: TD chance halved, FG chance +10%,
        turnover chance 5%
      Ball Hawk: baseline odds, turnover chance 10%
      Aggressive: TD chance x1.5, FG chance halved,
        turnover chance 20%

    Difficulty scaling on CPU scoring:
      Practice:   ~60% of Regular Season base odds
      Preseason:  ~80% of Regular Season base odds
      Regular:    full baseline
      Playoffs:   base odds x1.2 (CPU capitalizes more)
...
After missed field goal:
  - CPU starts at the kick spot (current ball position)
    or their own 20, whichever is further from the
    player's end zone
  - Triggers Defensive Posture
...
After CPU drive resolves:
  - Player gets ball at own 25, UNLESS the CPU drive
    ...
    starts at the turnover spot (better field position)
...
CPU field position coordinate note:
  ...
  player-perspective coordinates. The scoring odds table above
...
Ghost route lines (presnap/playcall):
  ...
  - Run plays: see ghost route rules in Game States section
...
Triggers:
  ...
  - First ghost route appearance on a run play: explains
    what the dashed/solid/faded lines mean
...
OL integrity per matchup is driven by the hold time system
(see Passing Mechanics → Pocket Integrity). On run plays,
the same matchup rolls determine whether gaps open or close.
Visual: OL dot color changes green→yellow→red.

================================================================
RUN YARDAGE RESOLUTION
================================================================

Run yardage is NOT a lookup table. It is resolved through the
physics-based runner decision system:

1. RPO READ: post-snap, player picks Hand Off / QB Keep /
   Quick Pass. Gap-open odds are driven by the matchup matrix
   (see RPO section above).

2. RUNNER PLACED ON FIELD: RB (or QB) is positioned at the
   formation spot and moves through the field based on player
   choices and defender positions.

3. RUNNER DECISIONS: player picks moves (Sprint, Hit the Hole,
   Juke, Cut Outside, Dive Forward) against live defender
   positions. Each move advances the ball carrier on the field.
   Yardage comes from the movement, not a formula.

4. CONTACT RESOLUTION: when defenders reach the carrier,
   contact system triggers (see Contact System below).

5. PLAY ENDS: when the carrier is tackled, goes out of bounds,
   scores, or runs out of moves.
...
Run direction uses forward cone gap-finding:
...
Phase-based system:
  - Phase advances with each QB action (drop back, look, etc.)
  - Receiver routes progress one waypoint per phase
  - Each Drop Back advances ALL receiver routes by a full
    phase tick
...
DB tracking speed (main balance knob):
  ...
  playtesting. The principle: defenders close on receivers
  faster at higher difficulties, so routes that get wide open
  in Practice are merely open or contested in Playoffs.
...
Look mechanic:
  ...
  - Throwing opposite of the look direction: +15% completion
  - This is emergent from WYSIWYG — the safety moves, which
    changes actual distances, which changes openness labels
...
Pocket integrity:
  ...
  HOLD TIME (rolled at snap):
    ...
    phases that lineman holds his block before he starts losing.
    ...
    phase 4. In Playoffs, a hold time of 1 means that edge is
    beaten on the first phase — the QB must react immediately.
    Most plays fall somewhere in between, with one side weaker
    than the other.
  ...
  LEFT/RIGHT AGGREGATION:
    Left pocket bar = average integrity of left-side matchups
      (LT, LG — typically OL positions 1-2)
    Right pocket bar = average integrity of right-side matchups
      (RG, RT — typically OL positions 4-5)
    Center contributes to both sides at 50%.
  ...
  DEFENSE-SPECIFIC POCKET BEHAVIOR:
    Base 4-3: most stable pocket. Standard 4 rushers,
      normal hold time distribution.
    Nickel: similar to Base but slightly faster degradation
      (DEs are quicker in this scheme).
    Blitz: fastest collapse. All hold times -1, plus LB
      rushers with hold time 1-2. High sack chance, but
      also leaves receivers uncovered.
    Cover 2: moderate pocket. Standard 4 rush, nothing
      special — the defense's strength is in coverage,
      not pressure.
    Goal Line: packed line, extra bodies near the ball.
      Hold times similar to Blitz on interior matchups,
      but edge rushers are wider (slower to reach QB).
...
Blitz mechanics:
  ...
  - All hold times reduced by 1, LB rushers get hold time 1-2
    (see Pocket Integrity for full details)
...
When QB throws to a target:
  ...
  - Completion roll based on openness + pocket integrity
...
Difficulty scaling on grounding risk:
  Practice:   low risk (grounding called rarely, ~20% when
              conditions are met)
...
Field goal distance = current ball position + 17 yards
(endzone depth + snap distance).
...
After a miss: Defensive Posture with CPU starting at kick spot
  or their own 20, whichever is further from player's end zone.
...
Where attributes are used:
  ...
  Speed → pursuit calculations via speed factor
    (WR 1.0, RB 0.9, TE 0.7, QB 0.75 — already defined
    in Contact System; these derive from the speed stat)
...
Options and base percentages:
  Spin Move:
    ...
    Degrades per successive broken tackle: -15% per contact
    (so base, then base-15%, base-30%, base-45%)
...
Fumble risk escalation:
  Base fumble rate varies by move and context.
...
Practice:
  ...
  - Tutorial overlays at each game phase (including
    Defensive Posture and ghost route explanations)
  ...
  - CPU scoring: ~60% of Regular odds
  - Audibles: unlimited
...
Preseason:
  ...
  - CPU scoring: ~80% of Regular odds
  - Audibles: unlimited
